...
function resolveWithTen(): Promise<number> {
...
Promise.resolve(2)
  .then((num: number) => num * num)
  .then((num: number) => num * 2)
  .then((num: number) => num + 5)
  .then((result: number) => console.log("8. Promise chain result:", result));

//9. Viết một Promise đọc một mảng sau 1 giây và lọc các số chẵn.
const filterEvenPromise: Promise<number[]> = new Promise((resolve) => {
