...
async function getHelloAsync(): Promise<void> {
...
async function handleTaskError(): Promise<void> {
  try {
...
async function fetchUsers(ids: number[]): Promise<void> {
...
async function fetchWithTimeout(id: number): Promise<object> {
  const timeoutPromise = new Promise((_, reject) => {
    setTimeout(() => {
      reject(new Error("Request timed out after 2 seconds"));
    }, 2000);
  });
  return Promise.race([fetchUser(id), timeoutPromise]) as Promise<object>;
}



// Export tất cả các hàm để có thể gọi từ file khác
export {
  getHelloAsync,
  runSimulatedTask,
  handleTaskError,
